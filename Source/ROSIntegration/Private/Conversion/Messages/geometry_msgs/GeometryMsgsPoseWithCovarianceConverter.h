...
public:
	...
	static bool _bson_extract_child_pose_with_covariance(bson_t *b, FString key, ROSMessages::geometry_msgs::PoseWithCovariance *p)
	{
		bool KeyFound = false;

		KeyFound = UGeometryMsgsPoseConverter::_bson_extract_child_pose(b, key + ".pose", &p->pose); 
		if (!KeyFound) 
			return false;
	...
	static void _bson_append_pose_with_covariance(bson_t *b, ROSMessages::geometry_msgs::PoseWithCovariance *t)
	{
		...
		_bson_append_double_tarray(b, "covariance", TArray<double>(t->covariance, 36));
	}
};
